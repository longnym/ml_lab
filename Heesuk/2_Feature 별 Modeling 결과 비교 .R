...
# 병렬처리 enable : core 1 -> 4
registerDoMC(cores = 4)
getDoParWorkers()

## Caret createDataPartition 이용
...
# grid search control (10-fold CV를 5회 반복하여 parameter grid search) 
...
################### II. 선별 Feature 활용 (Top-Tier + 2nd Tier) ############################# 
...
x = model.matrix(log.SalePrice ~ ., train.1.2.tier)[, -1]  # -1은 Index 제거 용
...
# 2-1) dataset 재정리 (Matrix화)
x = model.matrix(log.SalePrice ~ ., train.1.tier)[, -1]  # -1은 Index 제거 용
