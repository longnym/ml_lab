...
# 2. RF
rfFit <- train(log.SalePrice ~ ., data = df.train.all, method = "rf", 
                trControl = ctrl, preProc = c("center", "scale"))
rfFit
...
gbmFit <- train(log.SalePrice ~ ., data = df.train.all, method = "gbm", 
               trControl = ctrl, preProc = c("center", "scale"))
gbmFit
gbm.importance <- varImp(gbmFit, n.trees = 150)  # error
gbm.importance
...
# 
# # 1. PLS
# plsFit <- train(log.SalePrice ~ ., data = df.train, method = "pls", 
#                 trControl = ctrl, preProc = c("center", "scale"))
# plsFit
# pls.importance <- varImp(plsFit)
# pls.importance
# 
# # 2. RF
# rfFit <- train(log.SalePrice ~ ., data = df.train, method = "rf", 
#                trControl = ctrl, preProc = c("center", "scale"))
# rfFit
# rf.importance <- varImp(rfFit)
# rf.importance
# 
# # 3. Gradient Boosting
# gbmFit <- train(log.SalePrice ~ ., data = df.train, method = "gbm", 
#                 trControl = ctrl, preProc = c("center", "scale"))
# gbmFit
# gbm.importance <- varImp(gbmFit, n.trees = 150)  # error
# gbm.importance
