...
train_new <- tot.df.all[1:1457,]
validation_new <- tot.df.all[1458:2916,]
...
# Prediction (XGBoost + GBM + Random Forest)
pred <- (pred1 + pred2 + pred5) / 3

test <- read.csv('test.csv')
submit <- data.frame(Id = test$Id, SalePrice = pred)
write.csv(submit, 'submission.csv', quote = FALSE, row.names=FALSE)